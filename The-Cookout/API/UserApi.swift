...
class UserApi {
    ...
    func queryUsers(withText text: String, completion: @escaping (User) -> Void) {
        usersRef.queryOrdered(byChild: "username").queryStarting(atValue: text).queryEnding(atValue: text+"\u{f8ff}").queryLimited(toFirst: 10).observeSingleEvent(of: .value, with: {
            snapshot in
