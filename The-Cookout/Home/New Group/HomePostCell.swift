...
class HomePostCell<PostType>: DatasourceCell, ContentViewCell where PostType: Post {
    ...
    private func setDetails(post: Post) {
        ...
        guard let owner = post.userId else {return}
        optionsButton.isHidden = (owner != currentUser) ? true : false
        USERS_REF.child(owner).observeSingleEvent(of: .value) { (snapshot) in
            guard let ss = snapshot.value as? [String:Any] else {return}
            self.userProfileImageView.loadImage(urlString: ss["profileImageUrl"] as! String)
        }
    }
