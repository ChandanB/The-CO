...
class MessageSender: NSObject {
  ...
  fileprivate func sendTextMessage() {
    guard let toID = conversation?.chatID, let fromID = CURRENT_USER?.uid, let text = self.text else {
      self.mediaCount -= 1
    ...
    let reference = MESSAGES_REF.childByAutoId()
  ...
  fileprivate func sendPhotoMessage(object: MediaObject) {
    guard let toID = conversation?.chatID, let fromID = CURRENT_USER?.uid else {
      self.mediaUploadGroup.leave()
      return
    }
  ...
  fileprivate func sendVideoMessage(object: MediaObject) {
    guard let toID = conversation?.chatID, let fromID = CURRENT_USER?.uid, let path = object.fileURL else {
      self.mediaUploadGroup.leave()
      return
    }
    ...
    reference.updateChildValues(values) { (error, _) in
      ...
      if let isGroupChat = self.conversation?.isGroupChat, isGroupChat, let membersIDs = self.conversation?.chatParticipantsIDs {
        for memberID in membersIDs {
          let userMessagesRef = USER_MESSAGES_REF.child(memberID).child(toID).child(userMessagesFirebaseFolder)
          userMessagesRef.updateChildValues([messageID: 1])
        }
      } else {
        let userMessagesRef = USER_MESSAGES_REF.child(fromID).child(toID).child(userMessagesFirebaseFolder)
        userMessagesRef.updateChildValues([messageID: 1])

        let recipientUserMessagesRef = USER_MESSAGES_REF.child(toID).child(fromID).child(userMessagesFirebaseFolder)
  ...
  fileprivate func updateLastMessage(with messageID: String) {
    ...
    if let isGroupChat = conversation?.isGroupChat, isGroupChat {
      ...
      guard let toID = conversation?.chatID, let uID = CURRENT_USER?.uid else { return }

      let ref = USER_MESSAGES_REF.child(uID).child(toID).child(messageMetaDataFirebaseFolder)
      let childValues: [String: Any] = ["chatID": toID,
                                        "lastMessageID": messageID,
                                        "isGroupChat": isGroupChat]
      ref.updateChildValues(childValues)

      let ref1 = USER_MESSAGES_REF.child(toID).child(uID).child(messageMetaDataFirebaseFolder)
  ...
  fileprivate func incrementBadgeForReciever() {
    if let isGroupChat = conversation?.isGroupChat, isGroupChat {
      ...
      guard let toId = conversation?.chatID, let fromId = CURRENT_USER?.uid, toId != fromId else { return }
