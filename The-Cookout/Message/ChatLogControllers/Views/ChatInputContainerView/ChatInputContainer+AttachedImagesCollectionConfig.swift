...
extension ChatInputContainerView: UICollectionViewDataSource, UICollectionViewDelegate, UICollectionViewDelegateFlowLayout {
 ...
 @objc func removeButtonDidTap(sender: UIButton) {
  ...
  func deselectAsset(row: Int) {
      ...
      self.mediaPickerController?.delegate?.controller?(self.mediaPickerController!, didDeselectAsset: self.selectedMedia[row].phAsset!, at: indexPath)
  }

 
  func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
    let cell = attachedImages.dequeueReusableCell(withReuseIdentifier: selectedMediaCollectionCellID, for: indexPath) as? SelectedMediaCollectionCell ?? SelectedMediaCollectionCell()
