...
class InAppNotificationManager: NSObject {
    ...
    func removeAllObservers() {
        ...
        for handle in notificationHandle {
            notificationReference = USER_MESSAGES_REF.child(currentUserID).child(handle.chatID).child(messageMetaDataFirebaseFolder)
    ...
    func observersForNotifications(conversations: [Conversation]) {
        ...
        for conversation in self.conversations {
            guard let currentUserID = CURRENT_USER?.uid, let chatID = conversation.chatID else { continue }
