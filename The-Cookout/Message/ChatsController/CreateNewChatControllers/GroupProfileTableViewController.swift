...
class GroupProfileTableViewController: UITableViewController {
  ...
  fileprivate func setupTableView() {
    ...
    tableView.register(SocialPointUsersTableViewCell.self, forCellReuseIdentifier: selectedFlaconUsersCellID)
...
extension GroupProfileTableViewController {
  ...
  @objc func createGroupChat() {
    guard currentReachabilityStatus != .notReachable, let chatName = groupProfileTableHeaderContainer.name.text, let currentUserID = CURRENT_USER?.uid else {
      basicErrorAlertWith(title: basicErrorTitleForAlert, message: noInternetError, controller: self)
      return
    }
