...
class ChatLogController: UICollectionViewController {
  ...
  var isTyping: Bool {
    ...
    set {
      ...
      let typingData: NSDictionary = [CURRENT_USER!.uid: newValue] //??
      if localTyping {
        sendTypingStatus(data: typingData)
      } else {
        if let isGroupChat = conversation?.isGroupChat, isGroupChat {
          guard let currentUserID = CURRENT_USER?.uid, let conversationID = conversation?.chatID else {
            return
          }
          ...
          let userIsTypingRef = USER_MESSAGES_REF.child(currentUserID).child(conversationID).child(typingIndicatorDatabaseID)
          userIsTypingRef.removeValue()
        }
      }
    }
  ...
  func observeTypingIndicator () {
    ...
    if let isGroupChat = conversation?.isGroupChat, isGroupChat {
      ...
      let indicatorRemovingReference = USER_MESSAGES_REF.child(currentUserID).child(conversationID).child(typingIndicatorDatabaseID)
      indicatorRemovingReference.onDisconnectRemoveValue()
      typingIndicatorReference = USER_MESSAGES_REF.child(conversationID).child(currentUserID).child(typingIndicatorDatabaseID).child(conversationID)
  ...
  func updateMessageStatus(messageRef: DatabaseReference) {
    ...
    guard let uid = CURRENT_USER?.uid, currentReachabilityStatus != .notReachable else { return }
  ...
  func updateMessageStatusUIAfterDeletion(sentMessage: Message) {
    guard let uid = CURRENT_USER?.uid, currentReachabilityStatus != .notReachable,
    ...
    if messages.last!.toId == uid && self.messages.last?.status != messageStatusRead {
      let messagesRef = MESSAGES_REF.child(lastMessageUID)
  ...
  func setupTitleName() {
    guard let currentUserID = CURRENT_USER?.uid, let toId = conversation?.chatID else { return }
  ...
  func configureTitleViewWithOnlineStatus() {
    ...
    userStatusReference = USERS_REF.child(toId)
  ...
  func setRightBarButtonItem () {
    ...
    guard let uid = CURRENT_USER?.uid, let conversationID = conversation?.chatID, uid != conversationID else { return }
  ...
  @objc func getInfoAction() {
    ...
    if let isGroupChat = conversation?.isGroupChat, isGroupChat {
      ...
      if conversation?.admin != CURRENT_USER?.uid {
  ...
  override var inputAccessoryView: UIView? {
    get {
      if let membersIDs = conversation?.chatParticipantsIDs,
        let uid = CURRENT_USER?.uid, membersIDs.contains(uid) {
