...
class GroupAdminControlsTableViewController: UITableViewController {
  ...
  var conversationAdminID = String() {
    didSet {
      if conversationAdminID == CURRENT_USER?.uid {
  ...
  var groupAvatarURL = String() {
    didSet {
      ...
      groupProfileTableHeaderContainer.profileImageView.sd_setImage(with: URL(string: groupAvatarURL), placeholderImage: nil, options: [.continueInBackground, .scaleDownLargeImages], completed: { (_, _, _, _) in
  ...
  fileprivate func setupTableView() {
    ...
    tableView.register(SocialPointUsersTableViewCell.self, forCellReuseIdentifier: membersCellID)
  ...
  func observeMembersChanges() {
    ...
    membersAddingHandle = membersAddingReference.observe(.childAdded) { (snapshot) in
      ...
      let newMemberReference = USERS_REF.child(id)
      ...
      newMemberReference.observeSingleEvent(of: .value, with: { (snapshot) in
        ...
        let user = User(uid: snapshot.key, dictionary: dictionary)
          ...
          return member.uid == snapshot.key }) {
    ...
    membersRemovingHandle = membersAddingReference.observe(.childRemoved) { (snapshot) in
      guard let id = snapshot.value as? String else { return }
        ...
        Database.database().fetchUser(withUID: id, completion: { (member) in
            guard let memberIndex = self.members.firstIndex(of: member) else {
                return
            }
            self.tableView.beginUpdates()
  ...
  func isCurrentUserMemberOfCurrentGroup() -> Bool {
    let membersIDs = members.map({ $0.uid })
    guard let uid = CURRENT_USER?.uid, membersIDs.contains(uid) else { return false }
    return true
  }
  ...
  override func tableView(_ tableView: UITableView, canEditRowAt indexPath: IndexPath) -> Bool {
    guard indexPath.section == 1, members[indexPath.row].uid != conversationAdminID, members[indexPath.row].uid != CURRENT_USER!.uid else { return false }
    return true
  }
    ...
    if editingStyle == .delete {
        let membersIDs = self.members.map { $0.uid }
      ...
      informationMessageSender.sendInformatoinMessage(chatID: chatID, membersIDs: membersIDs, text: text )
        let memberID = members[indexPath.row].uid
  ...
  func changeAdministrator(shouldLeaveTheGroup: Bool) {
    ...
    Database.database().fetchUser(withUID: uid) { (user) in
        guard let index = self.members.firstIndex(of: user) else { return}
        let currentUserName = self.members[index].name
  ...
  func leaveTheGroup() {
    ...
    Database.database().fetchUser(withUID: uid) { (user) in
        guard let index = self.members.firstIndex(of: user) else {return}
        let memberName = self.members[index].name
        let text = "\(memberName) left the group"
        let reference = Database.database().reference().child("groupChats").child(self.chatID).child(messageMetaDataFirebaseFolder).child("chatParticipantsIDs").child(uid)
        reference.removeValue { (_, _) in
            var membersIDs = self.members.map({$0.uid})
  ...
  override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
    ...
    if indexPath.section == 0 {
      ...
      if cell.title.text == adminControls.last {
        cell.title.textColor = SocialPointPalette.dismissRed
      } else {
        cell.title.textColor = SocialPointPalette.defaultBlue
      }
      return cell

    } else {
      let cell = tableView.dequeueReusableCell(withIdentifier: membersCellID, for: indexPath) as? SocialPointUsersTableViewCell ?? SocialPointUsersTableViewCell()
      cell.selectionStyle = .default
      if members[indexPath.row].uid == conversationAdminID {
      ...
      if members[indexPath.row].uid == CURRENT_USER?.uid {
      ...
      let url = members[indexPath.row].thumbnailPhotoURL
